...
function calculateATSScore(
  ...
  const keywordScore = Math.min(
    25,
    (matchedCount / Math.max(jobKeywords.length, 1)) * 25,
  );
  score += keywordScore;
...
function generateImprovements(
  ...
  if (resume.skills.length < 10) {
    improvements.push(
      `Add more skills (currently have ${resume.skills.length}, aim for 10+)`,
    );
  }
  ...
  if (criticalMissing.length > 0) {
    improvements.push(`Incorporate key skills: ${criticalMissing.join(", ")}`);
  }
  ...
  if (
    ...
    !resumeText.includes("accomplishments")
  ) {
